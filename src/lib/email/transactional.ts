 ...
 *   EMAIL_FROM                -> e.g. "MailPilot <no-reply@yourdomain.com>"
...
function fromAddress(): string {
  return process.env.EMAIL_FROM || "MailPilot <onboarding@resend.dev>";
}
...
function layout(title: string, bodyHtml: string, cta?: { label: string; url: string }): string {
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:32px 0;"><tr><td align="center">
    <table width="480" cellpadding="0" cellspacing="0" style="background:#fff;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;">
      <tr><td style="padding:24px 28px;border-bottom:1px solid #eef0f2;font-weight:700;font-size:18px;color:#111;">MailPilot AI</td></tr>
      ...
      <tr><td style="padding:18px 28px;border-top:1px solid #eef0f2;color:#9ca3af;font-size:12px;">Sent by MailPilot AI. If this wasn't expected, you can ignore it.</td></tr>
    </table>
...
export function sendInviteEmail(params: { to: string; orgName: string; inviterName?: string | null; role: string }) {
  ...
  const html = layout(
    ...
    `<p>${params.inviterName ? `${escape(params.inviterName)} has` : "You've been"} invited you to join
     <strong>${escape(params.orgName)}</strong> on MailPilot AI as a <strong>${params.role.toLowerCase()}</strong>.</p>
     <p>Create your account with <strong>this email address</strong> and you'll be added automatically.</p>`,
    { label: "Accept invite", url }
  );
  return sendTransactionalEmail({ to: params.to, subject: `Join ${params.orgName} on MailPilot AI`, html });
}
...
export function sendExistingUserAddedEmail(params: { to: string; orgName: string; role: string }) {
  const html = layout(
    ...
    `<p>You now have <strong>${params.role.toLowerCase()}</strong> access to <strong>${escape(params.orgName)}</strong> on MailPilot AI.</p>`,
...
export function sendJoinApprovedEmail(params: { to: string; orgName: string }) {
  const html = layout(
    `You're in 🎉`,
    `<p>Your request to join <strong>${escape(params.orgName)}</strong> on MailPilot AI has been approved.</p>`,
    { label: "Open dashboard", url: `${appUrl()}/dashboard` }
  );
...
export function sendPasswordResetEmail(params: { to: string; token: string }) {
  ...
  const html = layout(
    "Reset your password",
    `<p>We received a request to reset your password for your MailPilot AI account.</p>
     <p>Click the button below to choose a new password. This link will expire in 1 hour.</p>
     <p>If you did not request a password reset, you can safely ignore this email.</p>`,
    { label: "Reset Password", url }
  );
  return sendTransactionalEmail({ to: params.to, subject: "Reset your MailPilot AI password", html });
}
...
export function sendVerificationEmail(params: { to: string; token: string }) {
  ...
  const html = layout(
    ...
    `<p>Thank you for signing up for MailPilot AI!</p>
     <p>Please click the button below to verify your email address and unlock your dashboard.</p>
     <p>If you did not create a MailPilot account, you can safely ignore this email.</p>`,
    { label: "Verify Email", url }
  );
  return sendTransactionalEmail({ to: params.to, subject: "Verify your MailPilot AI email address", html });
}
